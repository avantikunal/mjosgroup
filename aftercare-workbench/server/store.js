...
function enrichTicket(base) {
  ...
  return {
    ...
    chargeParty: decision.chargeParty,
    aiMode: decision.aiMode,
    ruleId: decision.ruleId,
    explanation: decision.explanation,
    requiresSeniorPlumberSignOff: decision.requiresSeniorPlumberSignOff,
...
function createTicket(input) {
  ...
  const base = {
    ...
    builderIfKnown: input.builderIfKnown || '',
    faultTypeId: input.faultTypeId,
    errorCode: input.errorCode || '',
    serialNumber: input.serialNumber || '',
    photos: input.photos || [],
    symptoms: input.symptoms || '',
    leakChecklist: input.leakChecklist || {},
    serviceCertsProvided: !!input.serviceCertsProvided,
